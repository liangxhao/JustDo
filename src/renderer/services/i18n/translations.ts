...
export const translations: Record<LanguageType, Record<string, string>> = {
  zh: {
    ...
    permissionModeFullDescription: '仅本次运行：命令和文件修改均无需批准',
    permissionModeFullConfirmTitle: '确认启用完全权限？',
    permissionModeFullConfirmDescription:
      'Agent 和定时任务将在本次运行中无需批准执行主机命令和修改文件。应用重启后权限会恢复为“请求批准”。',
  ...
  en: {
    ...
    permissionModeFullDescription: 'This run only: commands and file changes need no approval',
    permissionModeFullConfirmTitle: 'Enable full access?',
    permissionModeFullConfirmDescription:
      'For this run, the agent and scheduled tasks can execute host commands and change files without approval. Permissions reset to Ask after restart.',
