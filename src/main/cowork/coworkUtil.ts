  ...
  try {
    ...
    if (npmBinDir && existsSync(npmBinDir)) {
      ...
      if (existsSync(npxCliJs)) {
        ...
        if (process.platform === 'win32') {
          ...
          coworkLog(
            'INFO',
            'resolveNodeShim',
            `Created npx.cmd shim: ${npxCmd} (using env var JUSTDO_NPM_BIN_DIR)`,
          );
        }
      } else {
      ...
      if (existsSync(npmCliJs)) {
        ...
        // npm.cmd for Windows — uses %JUSTDO_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          ...
          coworkLog(
            'INFO',
            'resolveNodeShim',
            `Created npm.cmd shim: ${npmCmd} (using env var JUSTDO_NPM_BIN_DIR)`,
          );
        }
      } else {
...
function resolveWindowsGitBashPath(): string | null {
  ...
  // 2. Bundled PortableGit (preferred default in JustDo package)
...
function ensureWindowsBashUtf8InitScript(): string | null {
  try {
    ...
    const content = [
      '#!/usr/bin/env bash',
      '# Auto-generated by JustDo – switch Windows console code page to UTF-8',
...
function applyPackagedEnvOverrides(env: Record<string, string | undefined>): void {
  ...
  if (app.isPackaged && !env.JUSTDO_ELECTRON_PATH) {
    env.JUSTDO_ELECTRON_PATH = electronNodeRuntimePath;
  }
  ...
  if (process.platform === 'win32') {
    ...
    if (configuredBashPath && bashPath === configuredBashPath) {
      ...
      if (!configuredHealth.ok) {
        ...
        if (fallbackPath && fallbackPath !== configuredBashPath) {
          ...
          env.JUSTDO_GIT_BASH_RESOLUTION_ERROR = diagnostic;
          coworkLog('WARN', 'resolveGitBash', diagnostic);
          bashPath = null;
        }
      }
    ...
    if (bashPath) {
      ...
      delete env.JUSTDO_GIT_BASH_RESOLUTION_ERROR;
      ...
      env.JUSTDO_GIT_BASH_RESOLUTION_ERROR = truncateDiagnostic(diagnostic);
    }
  ...
  if (shouldInjectShim) {
    ...
    delete env.JUSTDO_NODE_SHIM_ACTIVE;
    coworkLog(
      'INFO',
      'resolveNodeShim',
...
function verifyNodeEnvironment(env: Record<string, string | undefined>): void {
        ...
        if (tool === 'node' && resolvedForExec) {
          try {
            ...
            if (process.platform === 'win32' && /\.cmd$/i.test(resolvedForExec)) {
              execTarget = env.JUSTDO_ELECTRON_PATH || process.execPath;
            }
  ...
  coworkLog('INFO', tag, `JUSTDO_ELECTRON_PATH=${env.JUSTDO_ELECTRON_PATH || '(not set)'}`);
  coworkLog('INFO', tag, `JUSTDO_NPM_BIN_DIR=${env.JUSTDO_NPM_BIN_DIR || '(not set)'}`);
...
export async function getEnhancedEnv(): Promise<Record<string, string | undefined>> {
  ...
  if (process.platform === 'win32' || env.JUSTDO_NODE_SHIM_ACTIVE === '1') {
    ...
    delete env.JUSTDO_ELECTRON_PATH;
  }
