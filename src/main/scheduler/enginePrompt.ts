import type { CoworkAgentEngine } from '../engine/types';
...
export function buildScheduledTaskEnginePrompt(engine: CoworkAgentEngine): string {
  if (engine === 'openclaw') {
    return [
      ...
      '- JustDo always retains completed scheduled-task results in its in-app result inbox. In-app retention is not a delivery channel and does not require `announce`.',
      '- Unless the user explicitly asks for an external IM/channel notification or webhook, create the task with `delivery: { mode: "none" }`. Never use `announce` merely to make a result visible inside JustDo.',
      '- Prefer an isolated agent-turn task with `delivery.mode: "none"` for ordinary reminders and recurring work that only need to appear in JustDo.',
