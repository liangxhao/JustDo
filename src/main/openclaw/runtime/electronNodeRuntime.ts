...
export const buildWindowsChildProcessPreload = (): string =>
  [
    ...
    '};',
    '// Preserve Node exec/execFile promisify semantics without bypassing windowsHide.',
    "for (const method of ['exec', 'execFile']) {",
    '  const wrapped = childProcess[method];',
    '  Object.defineProperty(wrapped, promisify.custom, { value: function (...args) {',
    '    let child;',
    '    const promise = new Promise((resolve, reject) => {',
    '      child = wrapped.call(this, ...args, (error, stdout, stderr) => {',
    '        if (error) {',
    '          error.stdout = stdout;',
    '          error.stderr = stderr;',
    '          reject(error);',
