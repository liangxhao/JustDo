...
export class CoworkRunner extends EventEmitter {
  ...
  private async runClaudeCodeLocal(
    ...
    coworkLog(
      'INFO',
      'runClaudeCodeLocal',
      `MCP env: GUCCIAI_ELECTRON_PATH=${envVars.GUCCIAI_ELECTRON_PATH || '(not set)'}`,
    );
    coworkLog(
      'INFO',
      'runClaudeCodeLocal',
    ...
    coworkLog(
      'INFO',
      'runClaudeCodeLocal',
      `MCP env: GUCCIAI_NPM_BIN_DIR=${envVars.GUCCIAI_NPM_BIN_DIR || '(not set)'}`,
    );
    ...
    if (process.platform === 'win32' && !envVars.CLAUDE_CODE_GIT_BASH_PATH) {
      const bashResolutionDiagnostic =
        typeof envVars.GUCCIAI_GIT_BASH_RESOLUTION_ERROR === 'string'
          ? envVars.GUCCIAI_GIT_BASH_RESOLUTION_ERROR.trim()
          : '';
      const errorMsg =
    ...
    if (app.isPackaged) {
      ...
      options.spawnClaudeCodeProcess = (spawnOptions: {
        ...
        const useElectronShim =
          ...
          isPackagedDarwin ||
          spawnOptions.env?.GUCCIAI_NODE_SHIM_ACTIVE === '1';
        const spawnEnv: NodeJS.ProcessEnv = {
          ...(spawnOptions.env ?? {}),
          ELECTRON_RUN_AS_NODE: '1',
        };
        if (useElectronShim) {
          spawnEnv.GUCCIAI_ELECTRON_PATH =
            spawnOptions.env?.GUCCIAI_ELECTRON_PATH || electronNodeRuntimePath;
        } else {
          delete spawnEnv.GUCCIAI_ELECTRON_PATH;
        }
        ...
        if (process.platform === 'win32' && isNodeLikeCommand) {
          ...
          spawnEnv.GUCCIAI_ELECTRON_PATH = electronNodeRuntimePath;
          coworkLog(
            'INFO',
            'runClaudeCodeLocal',
            ...
            switch (server.transportType) {
              case 'stdio': {
                ...
                if (process.platform === 'win32' && app.isPackaged && effectiveStdioCommand) {
                  ...
                  const npmBinDir = envVars.GUCCIAI_NPM_BIN_DIR;
    ...
    if (eventType === 'result') {
      ...
      if (subtype !== 'success') {
        const errors = Array.isArray(payload.errors)
          ? payload.errors
              .map(error =>
                typeof error === 'string' ? error.trim() : this.normalizeSdkError(error),
              )
