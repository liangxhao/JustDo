...
export function ensureElectronNodeShim(electronPath: string, npmBinDir?: string): string | null {
  try {
    ...
    const nodeShContent = [
      '#!/usr/bin/env bash',
      'if [ -z "${GUCCIAI_ELECTRON_PATH:-}" ]; then',
      '  echo "GUCCIAI_ELECTRON_PATH is not set" >&2',
      '  exit 127',
      'fi',
      'exec env ELECTRON_RUN_AS_NODE=1 "${GUCCIAI_ELECTRON_PATH}" "$@"',
      '',
    ].join('\n');
    ...
    if (process.platform === 'win32') {
      ...
      const nodeCmdContent = [
        '@echo off',
        'if "%GUCCIAI_ELECTRON_PATH%"=="" (',
        '  echo GUCCIAI_ELECTRON_PATH is not set 1>&2',
        '  exit /b 127',
        ')',
        'set ELECTRON_RUN_AS_NODE=1',
        '"%GUCCIAI_ELECTRON_PATH%" %*',
        '',
      ].join('\r\n');
    ...
    if (npmBinDir && existsSync(npmBinDir)) {
      ...
      if (existsSync(npxCliJs)) {
        ...
        if (process.platform === 'win32') {
          ...
          coworkLog(
            'INFO',
            'resolveNodeShim',
            `Created npx.cmd shim: ${npxCmd} (using env var GUCCIAI_NPM_BIN_DIR)`,
          );
        }
      } else {
      ...
      if (existsSync(npmCliJs)) {
        ...
        // npm.cmd for Windows — uses %GUCCIAI_NPM_BIN_DIR% env var to avoid
        // hardcoding paths that may contain non-ASCII chars (breaks GBK cmd.exe).
        if (process.platform === 'win32') {
          ...
          coworkLog(
            'INFO',
            'resolveNodeShim',
            `Created npm.cmd shim: ${npmCmd} (using env var GUCCIAI_NPM_BIN_DIR)`,
          );
        }
      } else {
...
function resolveWindowsGitBashPath(): string | null {
  ...
  // 2. Bundled PortableGit (preferred default in GucciAI package)
  ...
  // 5. Try `where bash` - but exclude WSL bash paths
  // WSL bash can be in:
  //   - C:\Windows\System32\bash.exe
  //   - C:\Users\...\AppData\Local\Microsoft\WindowsApps\bash.exe
  // Both should be excluded as they run in a separate Linux environment.
  const bashPaths = listWindowsCommandPaths('where bash');
  for (const bashPath of bashPaths) {
    const lower = bashPath.toLowerCase();
    // Exclude WindowsApps (Microsoft Store WSL) and System32\bash.exe (native WSL)
    if (
...
function ensureWindowsBashUtf8InitScript(): string | null {
  try {
    ...
    const content = [
      '#!/usr/bin/env bash',
      '# Auto-generated by GucciAI – switch Windows console code page to UTF-8',
...
function applyPackagedEnvOverrides(env: Record<string, string | undefined>): void {
  ...
  if (app.isPackaged && !env.GUCCIAI_ELECTRON_PATH) {
    env.GUCCIAI_ELECTRON_PATH = electronNodeRuntimePath;
  }
  ...
  if (process.platform === 'win32') {
    ...
    if (configuredBashPath && bashPath === configuredBashPath) {
      ...
      if (!configuredHealth.ok) {
        ...
        if (fallbackPath && fallbackPath !== configuredBashPath) {
          ...
          env.GUCCIAI_GIT_BASH_RESOLUTION_ERROR = diagnostic;
          coworkLog('WARN', 'resolveGitBash', diagnostic);
          bashPath = null;
        }
      }
    ...
    if (bashPath) {
      ...
      delete env.GUCCIAI_GIT_BASH_RESOLUTION_ERROR;
      ...
      env.GUCCIAI_GIT_BASH_RESOLUTION_ERROR = truncateDiagnostic(diagnostic);
    }
  ...
  if (shouldInjectShim) {
    ...
    if (shimDir) {
      ...
      env.GUCCIAI_NODE_SHIM_ACTIVE = '1';
      coworkLog(
        'INFO',
        'resolveNodeShim',
    ...
    delete env.GUCCIAI_NODE_SHIM_ACTIVE;
    coworkLog(
      'INFO',
      'resolveNodeShim',
...
function verifyNodeEnvironment(env: Record<string, string | undefined>): void {
        ...
        if (tool === 'node' && resolvedForExec) {
          try {
            ...
            if (process.platform === 'win32' && /\.cmd$/i.test(resolvedForExec)) {
              execTarget = env.GUCCIAI_ELECTRON_PATH || process.execPath;
            }
  ...
  coworkLog('INFO', tag, `GUCCIAI_ELECTRON_PATH=${env.GUCCIAI_ELECTRON_PATH || '(not set)'}`);
  coworkLog('INFO', tag, `GUCCIAI_NPM_BIN_DIR=${env.GUCCIAI_NPM_BIN_DIR || '(not set)'}`);
...
export async function getEnhancedEnv(
  ...
  env.GUCCIAI_SKILLS_ROOT = skillsRoot; // Alternative name for clarity
  if (process.platform === 'win32' || env.GUCCIAI_NODE_SHIM_ACTIVE === '1') {
    env.GUCCIAI_ELECTRON_PATH = getElectronNodeRuntimePath().replace(/\\/g, '/');
  } else {
    delete env.GUCCIAI_ELECTRON_PATH;
  }
