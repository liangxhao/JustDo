      ...
      if (systemNode) {
        if (nodeCommandType === 'node') {
          ...
          if (cliJs && fs.existsSync(cliJs)) {
            ...
            log(
              'INFO',
              `"${server.name}": using system Node.js "${systemNode}" + ${nodeCommandType}-cli.js (preferred over Electron runtime)`,
            );
          } else {
        ...
        if (nodeCommandType === 'node') {
          ...
          log(
            'WARN',
            `"${server.name}": no system Node.js found, falling back to Electron + npx-cli.js (may cause stdin issues)`,
          );
          ...
          log(
            'WARN',
            `"${server.name}": no system Node.js found, falling back to Electron + npm-cli.js (may cause stdin issues)`,
          );
        }
      }
    }
...
export class McpServerManager {
  ...
  private async startSingleServer(record: McpServerRecord): Promise<ManagedMcpServer | null> {
    ...
    if (record.transportType === 'stdio') {
      ...
      log(
        'INFO',
        `Starting "${record.name}" via stdio: command=${resolved.command}, args=${JSON.stringify(resolved.args)}`,
      );
    ...
    try {
      ...
      const stderrSummary =
        stderrChunks.length > 0 ? ` | stderr: ${stderrChunks.join(' ').slice(0, 500)}` : '';
