...
import { applyPortableGitRuntimeEnv, resolvePortableGitExecutables } from './portableGitRuntime';
...
test('exposes bundled MinGit on PATH without configuring a shell', () => {
  ...
  expect(env.PATH).toBe(`${path.join(root, 'cmd')};${existingPath}`);
  expect(env.Path).toBe(env.PATH);
  expect(env.JUSTDO_PORTABLE_GIT_ROOT).toBe(root);
  expect(env.JUSTDO_BASH_PATH).toBeUndefined();
...
test('supports the alternative MinGit git.exe layout accepted by packaging', () => {
  ...
  expect(executables).toEqual({
    root,
    gitPath: path.join(root, 'bin', 'git.exe'),
  });
  expect(env.PATH).toBe([path.join(root, 'bin'), 'C:\\Windows'].join(';'));
  expect(env.JUSTDO_BASH_PATH).toBeUndefined();
});

test('keeps PATH entries unique across casing differences and repeated application', () => {
  const root = createPortableGitFixture();
  const binDir = path.join(root, 'cmd');
  const env: Record<string, string | undefined> = {
...
test('does not advertise an incomplete MinGit runtime', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'justdo-portable-git-'));
  tempDirs.push(root);
  fs.mkdirSync(path.join(root, 'bin'), { recursive: true });
  const env: Record<string, string | undefined> = { PATH: 'C:\\Windows' };
