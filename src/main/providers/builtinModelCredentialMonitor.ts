...
import { type BuiltinModelCredential, getBuiltinModelCredentialRefreshAtSeconds } from './builtinModelCredential';
...
export const resolveBuiltinModelCredentialExpiryDelayMs = (
  ...
  nowMs = Date.now(),
): number => Math.max(1_000, getBuiltinModelCredentialRefreshAtSeconds(credential) * 1_000 - nowMs);
...
export class BuiltinModelCredentialMonitor {
  ...
  private scheduleExpiryRefresh(credential: BuiltinModelCredential | null): void {
    ...
    this.expiryTimer = null;
    if (!this.started) return;
    if (credential?.authType === 'api-key') return;
    this.expiryTimer = setTimeout(() => {
      this.expiryTimer = null;
      this.enqueueRefresh();
    }, credential ? resolveBuiltinModelCredentialExpiryDelayMs(credential) : LOGIN_RETRY_MS);
  }
}
