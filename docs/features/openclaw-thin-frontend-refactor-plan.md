...
Runtime patch 位于 `scripts/patches/v2026.8.2/`，只补齐锁定上游仍缺失的十三项产品能力。每个 patch 必须有版本锚点、幂等测试、职责说明和可删除条件；不得重做上游原生 task/history/approval/compaction；010/012 只让 host 为原生 exec/plugin approval 提供等待期限，011 只转发上游已有的 reviewer-only plugin approval detail，013 只修正暂停中止后的原生 Goal resume 准入，均不复制上游状态机。
