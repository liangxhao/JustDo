...
> 文件名保留历史审计日期以维持已有链接；本文已按 JustDo `v2026.8.27` / OpenClaw `v2026.8.1` 的实际代码重新核对，不再描述 2026-07-26 时的临时实现。
...
当前实现已经形成“Gateway 为执行与历史唯一真源、Main 负责产品命令/运行状态、Renderer 集中式 Gateway client 负责聊天数据面与可重建投影”的闭环。旧版最危险的全局流状态、双份 SQLite transcript、按文本猜测事件归属、终态后继续追加、单纯 append delta 等问题均已移除或由 session/run/sequence/lifecycle 约束替代。剩余风险主要集中在 Main/Renderer 两条 Gateway 消费路径的契约一致性、外部 Gateway 协议升级，以及超大工具输出只保留有限实时视图。
...
| Main 路由       | `src/main/engine/cowork/coworkEngineRouter.ts`              | 受理初始运行、停止与产品级运行查询                        |
| OpenClaw 适配   | `src/main/engine/openclaw/openclawRuntimeAdapter.ts`        | 初始 Gateway RPC、run 生命周期、审批与后台任务            |
| Main 广播       | `src/main/engine/cowork/coworkRuntimeForwarder.ts`          | 广播 activity/complete/error 等产品状态                   |
...
sequenceDiagram
  ...
  loop gateway stream
    G-->>A: lifecycle event
    A->>A: update product run status
    A-->>R: activity/complete/error
    G-->>C: chat/session wire event
    C-->>R: connection-ordered event
    R->>R: normalize and reduce Thinking/Tool/Content
    R->>R: scheduled render
  end
  G-->>A: final/aborted/error
  A-->>R: product run completion
  G-->>C: chat final + session.message
  R->>R: close turn, retain recent run guard
  R->>R: apply identified durable message
  R->>G: guarded history fallback when needed
  G-->>R: authoritative history
```
...
- `lastAgentSeq`：已接收的最大序号；Thinking/Tool 等稳定 owner 另有独立序号栅栏；
...
final 事件关闭活动 turn，补齐结束时间并完成仍在运行的适用 item。随后终态 run 进入近期集合。OpenClaw 先广播 final、后完成终态持久化，因此 Controller 按 100/400/1500/3000ms 有界重试历史，并绑定 session/run/generation；不能立即读一次旧快照后就宣告收敛。
...
连接状态与 run 状态分离。重连后通过 session/history 查询恢复权威消息；`lifecycleGeneration` 防止重启前缓存事件接入新生命周期。当前实现已删除 SQLite/Main transcript fallback；Gateway 历史不可用时执行连接恢复并保留已有的页面级有界状态，不能用产品缓存伪造完整工具时间线。
...
| 每条持久消息都重载整段历史     | session.message 按稳定身份即时合入，歧义才回退 |
| final 早于持久化导致结果不刷新 | 终态 history 有界退避并绑定 session/run 归属   |

## 10. 当前边界与风险

1. Gateway 原生事件只能由 Main adapter 或集中式 Renderer chat gateway 层消费；升级 OpenClaw 时必须同步审查两条路径和补丁，不能把新字段直接透传到展示组件。
2. Gateway 历史不可用时没有本地 transcript 兜底；页面只能保留当前内存投影并等待连接恢复，不能把产品 SQLite 元数据伪装成聊天历史。
...
- 切换会话后旧历史响应无效；
- Gateway 历史、optimistic 状态及断线恢复的来源边界；
- 计划工具在实时和历史中的一致投影；
