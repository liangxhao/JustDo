...
> 历史审计：问题结论、源码行号与测试结果仅适用于下文提交。部分路径属于当时参考的 OpenClaw 仓库；当前发送、停止与恢复机制见[Cowork](../architecture/04-cowork-system.md)和[Agent Engine](../architecture/05-agent-engine.md)，不要直接将本报告视为当前待修清单。
