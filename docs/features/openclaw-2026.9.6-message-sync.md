...
## 当前方案：撤销 017、018，使用原生恢复和历史投影

017、018 已从当前版本补丁清单删除，运行时须从锁定的原始包重新构建，剩余 22 个补丁。
不能通过对旧运行时逆向替换注入代码来卸载补丁。以下涉及 017 修复及 24 个补丁的
记录是已撤销方案的历史审计，不再代表当前部署要求。

018 移除后，混合工具消息直接使用原生 commentary fallback 投影及清洗规则。
例如实时 `Thinking → Commentary → Tool`，重新加载历史后可显示为
`Commentary → Thinking → Tool`。Renderer 保留 Gateway 返回顺序，不再要求历史
与实时块顺序完全一致，也不在前端补做重排。下文 23 个补丁的验证是移除 018 前的记录。
