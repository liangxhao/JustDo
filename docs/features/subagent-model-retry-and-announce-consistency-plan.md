...
> 本文按 JustDo `v2026.8.27`、OpenClaw `v2026.8.2` 重新核对。Task admission、queue、required-child join 与 completion delivery 已由上游原生实现；“当前模型请求无副作用时的同模型 retry”仍不是一项可宣称完整落地的通用能力。
...
OpenClaw v2026.8.2 原生负责 `sessions_spawn`、task ledger/event、原子 admission、排队、run timeout、required-child join 和 completion delivery。JustDo 通过 `tasks.list/get` 的版本化 wire validator 与稳定 DTO 展示父子关系，不再维护 managed join/FIFO/canonical patch 状态机。
...
| 能力                                    | 状态                          |
| --------------------------------------- | ----------------------------- |
| 原子 spawn admission                    | v2026.8.2 原生                |
| queued/running timeout 语义             | v2026.8.2 原生                |
| required child join/completion delivery | v2026.8.2 原生                |
| task ledger/event 与重启恢复            | v2026.8.2 原生 + app epoch 008 |
| task status/label 产品投影              | `tasks.list/get` Adapter DTO  |
| context overflow 收敛/归因              | v2026.8.2 原生                |
...
诊断记录 parent/child/task/run/request id、model/provider 错误分类、是否已有 delta/tool/commit 和 native delivery 状态；不记录 prompt/tool secret。Pristine tests 证明 queue/join 来自上游，`v2026_8_2` wire、`subagentGateway.test.ts` 和 adapter/goal tests 验证 consumer 映射。
