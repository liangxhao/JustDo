...
控制器入口继续拥有实例状态、生命周期与对外 API，领域模块处理对应操作。聊天控制器按会话、历史、压缩、事件恢复和进度卡划分；运行适配器按 Gateway 连接、事件、Plan/问答、Goal、历史和运行状态划分；浏览器桥接把页面快照与动作执行交给独立模块。领域模块声明明确的依赖接口，入口通过 `shared/app/propertyContext.ts` 创建一次读取当前值的访问器上下文，避免异步操作捕获过期的会话、连接或取消状态。模块不会接收整个控制器，也不新增消息持久化或转录缓存。

```mermaid
flowchart LR
  Caller[现有调用方] --> Entry[控制器入口与生命周期]
  Entry --> Domain[领域操作模块]
  Domain --> Context[显式依赖接口与实时访问器]
  Context --> State[入口拥有的当前状态和回调]
```

React 页面继续负责跨领域组合与保存；附件、浏览器面板、侧边问答、轮询及会话操作的 Hook 与所属领域共置。聊天样式从 Lit 组件移到同目录样式模块，保持原样式顺序和 Shadow DOM 作用域。翻译按领域同时维护中英文，原入口负责聚合。配置构建与配置同步写入分开，原同步入口保留已有导出。

这轮拆分保留原有状态所有权，尚未把各控制器改造成多个独立状态机。进一步调整状态所有权应单独进行，并验证会话切换、事件顺序、取消与重连行为。完整范围及验证记录见 [大文件拆分](../features/src-large-file-refactor.md)。
