...
JustDo 当前定位应是 OpenClaw 的桌面前端：负责 Electron 桌面体验、配置管理、权限交互、本地 UI 数据和 Artifact 预览，而不是重新实现 OpenClaw Runtime 的会话调度、Subagent 状态机或消息历史权威层。
...
2. JustDo SQLite 只保存本地产品数据、UI 元数据、缓存和权限审计。
3. JustDo 不再通过自然语言 announce 内容、toolCallId 猜测、session key 猜测来判断 Subagent 状态。
...
- OpenClaw 升级时容易静默失效。
- 问题很难判断是 OpenClaw 原生问题，还是 JustDo patch 后的问题。
...
- JustDo 同时拥有 SQLite `cowork_messages` 和 OpenClaw `chat.history`。
...
- 最终可考虑删除，或仅保留 UI 展开状态、备注等 JustDo 自有字段。
...
- 如果该层把 OpenClaw session key 映射成 JustDo session 并长期保存，容易形成另一套 session graph。
...
- 摸清 OpenClaw 已经提供哪些能力，避免 JustDo 自己重复实现。

任务：

1. 梳理当前 JustDo 调用的 OpenClaw Gateway API。
2. 确认以下能力是否稳定：
   - `chat.send`
   - `chat.abort`
   - `chat.history`
...
4. 如 OpenClaw 缺少结构化字段，优先形成上游 issue/PR，而不是在 JustDo 中猜测。
...
- UI 展示 Subagent 时，以 OpenClaw child session 为中心，而不是 JustDo 自建状态。
...
### 阶段 4：移除 JustDo Subagent 状态机

目标：

- JustDo 不再判断 Subagent 完成数量，也不再决定 Parent 是否应该恢复。
...
- JustDo 不再通过本地计数判断“还有几个 Subagent 没完成”。
...
- 将 OpenClaw 行为修复回归 OpenClaw，而不是长期留在 JustDo patch 层。
...
3. JustDo 旧状态机补偿类 patch，在对应状态机删除后移除。
...
- 中。依赖 OpenClaw 原生修复或 JustDo 不再依赖该路径。
...
- patch 删除必须放到最后。
- 删除前先确认 OpenClaw 上游修复或 JustDo 不再依赖该 announce 路径。
...
3. JustDo 不再维护 Subagent 完成计数。
4. JustDo 不再通过猜测 toolCallId/session label 绑定 child history。
