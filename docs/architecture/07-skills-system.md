...
Skills 是 JustDo 的扩展机制，每个 Skill 定义了一组特定场景下的工具和能力。JustDo 内置 17 个 Skill（由 OpenClaw Gateway 管理），覆盖文档生成、视频创作、数据分析、网络自动化等场景。
...
{
  "version": 1,
  "description": "Default skill configuration for JustDo",
  "defaults": {
...
| `scripts/install-openclaw-runtime.cjs (via patch-openclaw-runtime.cjs)` | 构建脚本（处理 Skills） |
...
│  ┌─────────────────┐  ┌─────────────────────────────────┐  │
│  │ resources/      │  │ skills/                          │  │
│  │ builtin-skills. │  │ (JustDo 内置 Skills)            │  │
│  │ json            │  │                                 │  │
│  └─────────────────┘  └─────────────────────────────────┘  │
│           │                          │                      │
│           │  ┌───────────────────────────────────────────┐ │
│           │  │ scripts/install-openclaw-runtime.cjs (via patch-openclaw-runtime.cjs)          │ │
│           └───────────────────────────────────────────────┘ │
│                              │                              │
└──────────────────────────────│──────────────────────────────┘
                               │ 构建时处理
...
│  ├── gateway.asar                                           │
│  ├── skills/                                                │
│  │   ├── create-plan/    (from JustDo)                     │
│  │   ├── docx/           (from JustDo)                     │
│  │   ├── web-search/     (from JustDo)                     │
│  │   └── ...                                                │
│  └─────────────────────────────────────────────────────────┘
```
...
{
  "version": 1,
  "description": "JustDo built-in skills configuration",
  "skills": [
...
在 `scripts/install-openclaw-runtime.cjs (via patch-openclaw-runtime.cjs)` 的步骤 `[4/7b]` 中：

1. 加载 `resources/builtin-skills.json` 配置
2. 若 `disableOpenClawDefaults=true`：删除 runtime 的 `skills/` 目录所有内容
3. 从 JustDo `skills/` 复制配置中指定的 Skills 到 runtime 的 `skills/`
