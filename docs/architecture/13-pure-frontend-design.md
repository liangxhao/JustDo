...
> **状态**：本设计在 v2026.5-v2026.6 已基本落地。JustDo 不再注入自定义 system prompt / AGENTS.md policy / per-agent workspace。当前边界参见 [openclaw-gateway-capability-matrix.md](openclaw-gateway-capability-matrix.md)。
...
- Skill 同步（从 JustDo resources/skills 目录到 OpenClaw state 目录）
