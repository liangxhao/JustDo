...
resolve_node_runtime() {
  ...
  if [ -n "${GUCCIAI_ELECTRON_PATH:-}" ] && [ -x "${GUCCIAI_ELECTRON_PATH}" ]; then
    NODE_CMD="$GUCCIAI_ELECTRON_PATH"
    NODE_ARGS=()
...
ensure_npm_available() {
  ...
  echo "  Please reinstall the web-search skill runtime from GucciAI."
  return 1
}
...
if ! resolve_node_runtime; then
  ...
  echo "  Please install Node.js, or run from GucciAI so scripts can use Electron runtime."
  exit 1
fi
