...
// Capability: make OpenClaw's explicit forced CLI memory rebuild recompute cached embeddings.
// Target: openclaw@2026.9.2 builtin memory manager source.
// Scope: copying the old embedding cache into the shadow reindex database only.
// Safety: only the native `reason: "cli"` plus `force: true` pair bypasses cache seeding; automatic
// provider fallback and other forced maintenance retain upstream cache behavior. The original
// database stays intact until the atomic publish succeeds.
// Remove when: upstream makes an explicit forced CLI reindex bypass the old embedding cache.
...
const MARKER = 'JUSTDO_MEMORY_FORCE_REEMBED_OPT_IN_V2026_9_2';
const FUNCTION_SIGNATURE = 'async runInPlaceReindex(params) {';
const CACHE_SEED = 'await this.seedEmbeddingCache(originalDb);';
const PATCHED_CACHE_SEED = `if (!(params?.force === true && params?.reason === "cli")) ${CACHE_SEED} // ${MARKER}`;
