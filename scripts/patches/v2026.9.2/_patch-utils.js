...
function assertCurrentPatchContract(content, contract, description, required = true) {
  const versionMarker = contract.lastIndexOf('_V');
  if (versionMarker < 0) throw new Error(`${description}: invalid patch contract ${contract}`);
  const family = contract.slice(0, versionMarker).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const markers = content.match(new RegExp(`${family}_V\\d+_\\d+_\\d+`, 'g')) ?? [];
  const historical = markers.find(marker => marker !== contract);
  if (historical) {
    throw new Error(`${description}: historical or partial patch contract ${historical}`);
  }
  if (required && !markers.includes(contract)) {
    throw new Error(`${description}: historical or partial patch contract; missing ${contract}`);
  }
}
