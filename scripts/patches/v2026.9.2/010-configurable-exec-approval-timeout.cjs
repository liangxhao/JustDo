...
// Target: openclaw@2026.9.2 exec runtime, agent Gateway client, and approval manager.
...
const MARKERS = {
  defaults: 'JUSTDO_CONFIGURABLE_EXEC_APPROVAL_TIMEOUT_DEFAULTS_V2026_9_2',
  gateway: 'JUSTDO_CONFIGURABLE_EXEC_APPROVAL_TIMEOUT_GATEWAY_V2026_9_2',
  manager: 'JUSTDO_CONFIGURABLE_EXEC_APPROVAL_TIMEOUT_MANAGER_V2026_9_2',
  request: 'JUSTDO_CONFIGURABLE_EXEC_APPROVAL_REQUEST_TIMEOUT_V2026_9_2',
  wait: 'JUSTDO_CONFIGURABLE_EXEC_APPROVAL_TIMEOUT_WAIT_V2026_9_2',
};
...
const MANAGER_DIRECT_EXPIRY_PATTERN =
  /((?:(?:const|let)\s+)?[A-Za-z_$][\w$]*\s*=\s*)resolveExpiresAtMsFromDurationMs\((resolve(?:Approval|Timer)TimeoutMs)\(([A-Za-z_$][\w$]*)((?:,\s*1)?)\),\s*\{\s*nowMs:\s*([A-Za-z_$][\w$]*)\s*\}\)/g;
const MANAGER_RESOLVED_EXPIRY_PATTERN =
  /(((?:(?:const|let)\s+)?([A-Za-z_$][\w$]*)\s*=\s*)resolve(?:Approval|Timer)TimeoutMs\(([A-Za-z_$][\w$]*)(?:,\s*1)?\);\s*)((?:(?:const|let)\s+)?[A-Za-z_$][\w$]*\s*=\s*)resolveExpiresAtMsFromDurationMs\(\3,\s*\{\s*nowMs:\s*([A-Za-z_$][\w$]*)\s*\}\)/g;
...
const PATCHED_MANAGER_DIRECT_PATTERN = new RegExp(
  `((?:(?:const|let)\\s+)?[A-Za-z_$][\\w$]*\\s*=\\s*)process\\.env\\.${ENV_NAME}===["']0["']&&([A-Za-z_$][\\w$]*)===Number\\.MAX_SAFE_INTEGER\\?Number\\.MAX_SAFE_INTEGER:resolveExpiresAtMsFromDurationMs\\(resolve(?:Approval|Timer)TimeoutMs\\(\\2(?:,\\s*1)?\\),\\s*\\{\\s*nowMs:\\s*([A-Za-z_$][\\w$]*)\\s*\\}\\)\\/\\*${MARKERS.manager}\\*\\/`,
  'g',
);
const PATCHED_MANAGER_RESOLVED_PATTERN = new RegExp(
  `(?:(?:const|let)\\s+)?([A-Za-z_$][\\w$]*)\\s*=\\s*resolve(?:Approval|Timer)TimeoutMs\\(([A-Za-z_$][\\w$]*)(?:,\\s*1)?\\);\\s*((?:(?:const|let)\\s+)?[A-Za-z_$][\\w$]*\\s*=\\s*)process\\.env\\.${ENV_NAME}===["']0["']&&\\2===Number\\.MAX_SAFE_INTEGER\\?Number\\.MAX_SAFE_INTEGER:resolveExpiresAtMsFromDurationMs\\(\\1,\\s*\\{\\s*nowMs:\\s*([A-Za-z_$][\\w$]*)\\s*\\}\\)\\/\\*${MARKERS.manager}\\*\\/`,
  'g',
);
...
const BUNDLE_MANAGER_DIRECT_EXPIRY_PATTERN =
  /[A-Za-z_$][\w$]*=process\.env\.JUSTDO_EXEC_APPROVAL_TIMEOUT_MS===["']0["']&&([A-Za-z_$][\w$]*)===Number\.MAX_SAFE_INTEGER\?Number\.MAX_SAFE_INTEGER:resolveExpiresAtMsFromDurationMs\(resolve(?:Approval|Timer)TimeoutMs\(\1(?:,1)?\),\{nowMs:[A-Za-z_$][\w$]*\}\)/g;
const BUNDLE_MANAGER_RESOLVED_EXPIRY_PATTERN =
  /([A-Za-z_$][\w$]*)=resolve(?:Approval|Timer)TimeoutMs\(([A-Za-z_$][\w$]*)(?:,1)?\);[A-Za-z_$][\w$]*=process\.env\.JUSTDO_EXEC_APPROVAL_TIMEOUT_MS===["']0["']&&\2===Number\.MAX_SAFE_INTEGER\?Number\.MAX_SAFE_INTEGER:resolveExpiresAtMsFromDurationMs\(\1,\{nowMs:[A-Za-z_$][\w$]*\}\)/g;
...
function transformManager(content, filePath) {
  if (isGatewayBundlePath(filePath)) {
    const normalized = normalizeJustDoGatewayBundle(content, filePath);
    const bundleCount =
      matchCount(normalized, BUNDLE_MANAGER_DIRECT_EXPIRY_PATTERN) +
      matchCount(normalized, BUNDLE_MANAGER_RESOLVED_EXPIRY_PATTERN);
    if (bundleCount !== 1) {
      throw new Error(
        `${filePath}: historical or partial approval manager indefinite expiry bundle contract ` +
          `detected; target count is ${bundleCount}, expected 1`,
      );
    }
    return content;
  }
  const originalCount =
    matchCount(content, MANAGER_DIRECT_EXPIRY_PATTERN) +
    matchCount(content, MANAGER_RESOLVED_EXPIRY_PATTERN);
  const patchedCount =
    matchCount(content, PATCHED_MANAGER_DIRECT_PATTERN) +
    matchCount(content, PATCHED_MANAGER_RESOLVED_PATTERN);
  if (
    assertSingleContract(
      content,
      filePath,
  ...
  return content
    .replace(
      MANAGER_RESOLVED_EXPIRY_PATTERN,
      (
        _match,
        resolvedStatement,
        _resolvedAssignment,
        ...
        `${resolvedStatement}${assignment}process.env.${ENV_NAME}==="0"&&${timeoutName}===Number.MAX_SAFE_INTEGER?Number.MAX_SAFE_INTEGER:resolveExpiresAtMsFromDurationMs(${resolvedName},{nowMs:${nowName}})/*${MARKERS.manager}*/`,
    )
    .replace(
      MANAGER_DIRECT_EXPIRY_PATTERN,
      (_match, assignment, resolver, timeoutName, resolverSuffix, nowName) =>
        `${assignment}process.env.${ENV_NAME}==="0"&&${timeoutName}===Number.MAX_SAFE_INTEGER?Number.MAX_SAFE_INTEGER:resolveExpiresAtMsFromDurationMs(${resolver}(${timeoutName}${resolverSuffix}),{nowMs:${nowName}})/*${MARKERS.manager}*/`,
    );
}
