...
// Target: openclaw@2026.9.2 managed outgoing media preparation, HTTP disposition, and chat history.
...
const MARKER = 'JUSTDO_TRUSTED_LOCAL_FILE_MEDIA_V2026_9_2';
const PROJECTION_MARKER = 'JUSTDO_RETAIN_ASSISTANT_MEDIA_URLS_V2026_9_2';
...
const PATCHED_CONTENT_TYPE_PATTERN =
  /((?:(?:const|let)\s+|,)\s*([A-Za-z_$][\w$]*)\s*=\s*([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*)\.contentType\s*\?\?\s*([A-Za-z_$][\w$]*)\.mimeType)\s*\?\?\s*\(\s*([A-Za-z_$][\w$]*)\s*&&\s*\4\.trustedLocal\s*\?\s*(["'`])application\/octet-stream\6\s*:\s*(?:void\s+0|undefined)\s*\)\s*;(?:\/\*JUSTDO_TRUSTED_LOCAL_FILE_MEDIA_V2026_9_2\*\/)?/gu;
...
const PATCHED_PROJECTION_RETURN_PATTERN =
  /return\s*\{\s*changed:\s*(?:false|!1)\s*,\s*urls(?:\s*:\s*([A-Za-z_$][\w$]*))?\s*\}\s*;(?:\/\*JUSTDO_RETAIN_ASSISTANT_MEDIA_URLS_V2026_9_2\*\/)?/gu;
