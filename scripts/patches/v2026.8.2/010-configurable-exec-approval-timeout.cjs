...
// Target: openclaw@2026.8.2 exec runtime, agent Gateway client, and approval manager.
...
const MARKERS = {
  defaults: 'JUSTDO_CONFIGURABLE_EXEC_APPROVAL_TIMEOUT_DEFAULTS_V2026_8_2',
  gateway: 'JUSTDO_CONFIGURABLE_EXEC_APPROVAL_TIMEOUT_GATEWAY_V2026_8_2',
  manager: 'JUSTDO_CONFIGURABLE_EXEC_APPROVAL_TIMEOUT_MANAGER_V2026_8_2',
  request: 'JUSTDO_CONFIGURABLE_EXEC_APPROVAL_REQUEST_TIMEOUT_V2026_8_2',
  wait: 'JUSTDO_CONFIGURABLE_EXEC_APPROVAL_TIMEOUT_WAIT_V2026_8_2',
};
...
const MANAGER_RESOLVED_EXPIRY_PATTERN =
  /(((?:(?:const|let)\s+)?([A-Za-z_$][\w$]*)\s*=\s*)resolveApprovalTimeoutMs\(([A-Za-z_$][\w$]*)\);\s*)((?:(?:const|let)\s+)?[A-Za-z_$][\w$]*\s*=\s*)resolveExpiresAtMsFromDurationMs\(\3,\s*\{\s*nowMs:\s*([A-Za-z_$][\w$]*)\s*\}\)/g;
...
const PATCHED_MANAGER_RESOLVED_PATTERN = new RegExp(
  `(?:(?:const|let)\\s+)?([A-Za-z_$][\\w$]*)\\s*=\\s*resolveApprovalTimeoutMs\\(([A-Za-z_$][\\w$]*)\\);\\s*((?:(?:const|let)\\s+)?[A-Za-z_$][\\w$]*\\s*=\\s*)process\\.env\\.${ENV_NAME}===["']0["']&&\\2===Number\\.MAX_SAFE_INTEGER\\?Number\\.MAX_SAFE_INTEGER:resolveExpiresAtMsFromDurationMs\\(\\1,\\s*\\{\\s*nowMs:\\s*([A-Za-z_$][\\w$]*)\\s*\\}\\)\\/\\*${MARKERS.manager}\\*\\/`,
  'g',
);
...
const BUNDLE_MANAGER_RESOLVED_EXPIRY_PATTERN =
  /([A-Za-z_$][\w$]*)=resolveApprovalTimeoutMs\(([A-Za-z_$][\w$]*)\);[A-Za-z_$][\w$]*=process\.env\.JUSTDO_EXEC_APPROVAL_TIMEOUT_MS===["']0["']&&\2===Number\.MAX_SAFE_INTEGER\?Number\.MAX_SAFE_INTEGER:resolveExpiresAtMsFromDurationMs\(\1,\{nowMs:[A-Za-z_$][\w$]*\}\)/g;
