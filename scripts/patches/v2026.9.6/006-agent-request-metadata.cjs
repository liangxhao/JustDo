...
// Target: pristine openclaw@2026.9.6, which does not publish these fields to provider payloads.
...
const CONTRACT = 'JUSTDO_AGENT_REQUEST_METADATA_AND_HIDDEN_TURNS_V2026_9_6';
...
function expectedCounts(runtimeDir) {
  ...
  return {
    schema: withBundle ? 4 : 3,
    registration: withBundle ? 2 : 1,
    stream: withBundle ? 4 : 3,
  };
}
...
function resolveAccessorImport(runtimeDir) {
  const files = findFilesContaining(runtimeDir, ['function loadSessionEntry(scope)', 'export {'], {
    includeBundle: false,
  }).filter(filePath => !['worker.mjs', 'sqlite-store.worker.mjs'].includes(path.basename(filePath)));
  if (files.length !== 1)
