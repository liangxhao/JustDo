...
// Target: openclaw@2026.9.6 managed outgoing media preparation, HTTP disposition, and chat history.
...
const MARKER = 'JUSTDO_TRUSTED_LOCAL_FILE_MEDIA_V2026_9_6';
const PROJECTION_MARKER = 'JUSTDO_RETAIN_ASSISTANT_MEDIA_URLS_V2026_9_6';
const DOCUMENT_SET_PATTERN =
  /\bMANAGED_DOCUMENT_MIME_TYPES\s*=\s*(?:\/\*[^*]*\*\/\s*)?new Set\(\[/gu;
const ORIGINAL_DOCUMENT_MIME_PATTERN =
  /(["'`])application\/json\1\s*,(\s*)(["'`])application\/msword\3/gu;
const PATCHED_DOCUMENT_MIME_PATTERN =
  /(["'`])application\/json\1\s*,(\s*)(["'`])application\/octet-stream\3\s*,(\s*)(["'`])application\/msword\5/gu;
...
const PATCHED_CONTENT_TYPE_PATTERN =
  /((?:(?:const|let)\s+|,)\s*([A-Za-z_$][\w$]*)\s*=\s*([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*)\.contentType\s*\?\?\s*([A-Za-z_$][\w$]*)\.mimeType)\s*\?\?\s*\(\s*([A-Za-z_$][\w$]*)\s*&&\s*\4\.trustedLocal\s*\?\s*(["'`])application\/octet-stream\6\s*:\s*(?:void\s+0|undefined)\s*\)\s*;(?:\/\*JUSTDO_TRUSTED_LOCAL_FILE_MEDIA_V2026_9_6\*\/)?/gu;
...
function isExactPatchedState(state, markerCount, filePath) {
  if (
    ...
    !state.disposition.body.includes('buildAssistantMediaContentDisposition(')
  ) {
    return false;
  }
...
function transformManagedMedia(content, filePath) {
  ...
  if (
    ...
    throw new Error(`${filePath}: trusted local file media pristine anchors are ambiguous: ${state.originalDocument.length}/${state.originalContentType.length}/${state.originalKindGuard.length}/${state.originalDisposition.length}`);
  }
  ...
  const documentBody = state.documentSet.body.replace(
    ...
    `${documentMatch[1]}application/json${documentMatch[1]},${documentMatch[2]}` +
      ...
      `${documentMatch[3]}application/msword${documentMatch[3]}`,
  );
...
const PATCHED_PROJECTION_RETURN_PATTERN =
  /return\s*\{\s*changed:\s*(?:false|!1)\s*,\s*urls(?:\s*:\s*([A-Za-z_$][\w$]*))?\s*\}\s*;(?:\/\*JUSTDO_RETAIN_ASSISTANT_MEDIA_URLS_V2026_9_6\*\/)?/gu;
