...
// Target: pristine openclaw@2026.9.6, whose host sanitizer blocks both environment variables.
...
const CAPTURE_MARKER = 'JUSTDO_V2026_9_6_MANAGED_PIP_CONFIG_CAPTURE';
const RESTORE_MARKER = 'JUSTDO_V2026_9_6_MANAGED_PIP_CONFIG_RESTORE';
...
function applyPatch(runtimeDir) {
  ...
  if (candidates.length !== 2) throw new Error('Expected host and worker Python sanitizers');
  const changed = [];
...
function verifyPatch(runtimeDir) {
  ...
  if (sourceCandidates.length !== 2) throw new Error('Expected patched host and worker Python sanitizers');
