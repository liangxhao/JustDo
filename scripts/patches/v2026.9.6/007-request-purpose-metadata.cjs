...
// Target: pristine openclaw@2026.9.6, which sends both as ordinary model completions.
...
const CONTRACT = 'JUSTDO_REQUEST_PURPOSE_METADATA_V2026_9_6';
const COMPACTION_MARKER = `${CONTRACT}: context compaction requests`;
const SIMPLE_MARKER = `${CONTRACT}: model-backed reviewer requests`;
const REVIEWER_MARKER = `${CONTRACT}: reviewer session lookup through v9.2 SQLite state`;
...
const REVIEWER_SESSION_BLOCK = `// ${REVIEWER_MARKER}.
...
function isMonolith(filePath) {
  return (
    ...
    ['worker.mjs', 'sqlite-store.worker.mjs'].includes(path.basename(filePath))
  );
}
...
function resolveProviderStreamChunk(runtimeDir) {
  const files = findFilesContaining(runtimeDir, ['function streamWithPayloadPatch(', 'export {'], {
    includeBundle: false,
  }).filter(filePath => !filePath.includes(path.sep + 'worker' + path.sep));
  if (files.length !== 1)
...
function resolveSessionAccessorChunk(runtimeDir) {
  ...
  if (!exportAlias) throw new Error('reviewer SQLite accessor export alias is missing');
...
function installPayloadHelper(content, runtimeDir, filePath, targetIndex, block) {
  ...
  if (!isMonolith(filePath)) {
    ...
    updated = `import { z as streamWithPayloadPatch } from "./${chunk}";\n${updated}`;
...
function patchCompaction(content, runtimeDir, filePath) {
  ...
  if (content.includes(`function ${COMPACTION_HELPER}(`)) {
    if (!isGatewayBundlePath(filePath) && !content.includes(COMPACTION_MARKER))
      throw new Error(`${filePath}: historical or partial compaction purpose helper`);
    return content;
  }
  const classPattern =
...
function patchSimpleCompletion(content, runtimeDir, filePath) {
  assertCurrentPatchContract(content, CONTRACT, filePath, false);
  const functionName = 'completePreparedModel';
  ...
  const completionModel =
    /\b(?:let\s+)?([A-Za-z_$][\w$]*)\s*=\s*(?:getModelCompletionTransport\([^)]*\)\s*\?\?\s*)?prepareModelForSimpleCompletion\(/.exec(
      range.body,
    )?.[1];
...
function patchExecReviewer(content, runtimeDir, filePath) {
  ...
  if (!updated.includes(`function ${REVIEWER_SESSION_HELPER}(`)) {
    if (isMonolith(filePath)) {
      ...
      const accessor = resolveSessionAccessorChunk(runtimeDir);
      updated = `import { ${accessor.exportAlias} as loadJustDoReviewerSessionEntry } from "./${accessor.fileName}";\n${updated}`;
...
function verifyPatch(runtimeDir) {
  ...
  for (const filePath of files.compaction) {
    ...
    for (const required of [
      ...
      ...(isGatewayBundlePath(filePath) ? [] : [COMPACTION_MARKER]),
  ...
  for (const filePath of files.simple) {
    ...
    for (const required of [
      ...
      ...(isGatewayBundlePath(filePath) ? [] : [SIMPLE_MARKER]),
  ...
  for (const filePath of files.reviewer) {
    ...
    for (const required of [
      ...
      ...(isGatewayBundlePath(filePath) ? [] : [REVIEWER_MARKER]),
