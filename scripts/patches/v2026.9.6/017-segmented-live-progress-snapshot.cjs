...
// Target: openclaw@2026.9.6 progress snapshots and their Gateway visibility boundary.
...
const MARKER = 'JUSTDO_SEGMENTED_LIVE_PROGRESS_V2026_9_6';
const HELPER = 'justdoRetainLiveProgressSegment';

function justdoRetainLiveProgressSegment(progressSnapshot, progressEvent) {
  const progressContract = 'JUSTDO_SEGMENTED_LIVE_PROGRESS_V2026_9_6';
...
function transformThinkingBoundary(content, filePath) {
  if (['worker.mjs', 'sqlite-store.worker.mjs'].includes(path.basename(filePath)))
  ...
  const arrow = /const emitReasoningStream = \((\w+), fallback\) => \{/u.exec(body);
