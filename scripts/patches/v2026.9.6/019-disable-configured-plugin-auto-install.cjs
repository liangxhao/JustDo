...
// Target: openclaw@2026.9.6 configured-plugin repair lifecycle.
...
const MARKER = 'JUSTDO_DISABLE_CONFIGURED_PLUGIN_AUTO_INSTALL_V2026_9_6';
...
function replacementSource(
  includeMarker,
  persistenceFunctionName = 'writePersistedInstalledPluginIndexInstallRecordsWithLease',
) {
  return [
    `async function ${FUNCTION_NAME}(params, lease) {`,
...
function assertTargetShape(runtimeDir, files) {
  ...
  // root Gateway bundle.
  const expectedSourceCount = bundleExists && sourceTargets.length === 0 ? 0 : 3;
...
function transform(content, filePath) {
  ...
  const persistenceFunctionMatches = [
    ...functionSource.matchAll(/\b(writePersistedInstalledPluginIndexInstallRecordsWithLease\d*)\s*\(/gu),
  ];
