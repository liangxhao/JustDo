...
// Target: openclaw@2026.9.6 native chat history sanitization.
...
const MARKER = 'JUSTDO_ORDERED_COMMENTARY_HISTORY_V2026_9_6';
const ORIGINAL_NAME = 'sanitizeChatHistoryMessages';
// Exact locked npm worker artifact (its locals are pre-minified).
const NATIVE_WORKER = "function sanitizeChatHistoryMessages(Ot,Zt=DEFAULT_CHAT_HISTORY_TEXT_MAX_CHARS,_n){if(Ot.length===0)return Ot;let Dn=!1,kn=[];for(let Pn of Ot){let Ot=Pn;if(_n?.includeCommentaryFallbacks===!0){let _n=projectAssistantCommentaryFallbacks(Ot,Zt);Ot=_n.message,Dn||=Ot!==Pn;for(let Ot of _n.fallbacks){Dn=!0;let _n=hasTranscriptMediaFacts(asOptionalRecord(Ot)??{});if(!_n&&shouldDropAssistantHistoryMessage(Ot))continue;let Pn=sanitizeChatHistoryMessage(Ot,Zt);(_n||!shouldDropAssistantHistoryMessage(Pn.message))&&kn.push(Pn.message)}}if(shouldDropAssistantHistoryMessage(Ot)){Dn=!0;continue}let Fn=sanitizeChatHistoryMessage(Ot,Zt);if(Dn||=Fn.changed,Fn.changed&&shouldDropAssistantHistoryMessage(Fn.message)){Dn=!0;continue}kn.push(Fn.message)}return Dn?kn:Ot}";
...
function sanitizeChatHistoryMessages(
  ...
  if (opts?.includeCommentaryFallbacks !== true)
    return JUSTDO_ORDERED_COMMENTARY_HISTORY_V2026_9_6(messages, maxChars, opts);
  let changed = false;
  const next = [];
  for (const message of messages) {
    ...
    if (
      ...
      const fallbacks = projectAssistantCommentaryFallbacks(message, maxChars).fallbacks;
    ...
    const projected = JUSTDO_ORDERED_COMMENTARY_HISTORY_V2026_9_6(
      [ordered],
      maxChars,
