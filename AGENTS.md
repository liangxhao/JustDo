...
GucciAI is an Electron + React desktop application with two primary modes:
...
1. **登录：** 打开系统浏览器 → Portal 登录页 → URS 登录成功 → deep link `gucciai://auth/callback?code=<authCode>`
...
- 登录流程：`src/main/main.ts`（deep link 处理 `gucciai://` 协议）
...
- Database file: `gucciai.sqlite` in user data directory
...
- Platform-specific identifiers passed through from external sources (e.g., API response fields, user config values).
...
- Scope (optional): the affected area, e.g. `feat(cowork):`, `fix(skill):`
