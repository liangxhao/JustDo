// Browser assistant extension service worker.
//
// Thin transport between the extension relay (loopback WebSocket) and
...
// shared tab group in sync. Membership in that group is the user-visible
// consent boundary: only grouped tabs are reported to (and driven by) the app.
import {
...
const TAB_GROUP_TITLE = chrome.runtime.getManifest().name;
...
async function findSharedGroups() {
  try {
    return await chrome.tabGroups.query({ title: TAB_GROUP_TITLE });
  } catch {
    return [];
  }
}
...
async function listSharedTabs() {
  const groups = await findSharedGroups();
  const tabs = [];
...
async function addTabToSharedGroup(tabId) {
...
async function removeTabFromSharedGroup(tabId) {
  try {
...
async function attachDebugger(tabId) {
  if (!(await isTabShared(tabId))) {
    throw new Error(`tab ${tabId} is not in the ${TAB_GROUP_TITLE} tab group`);
  }
...
chrome.debugger.onDetach.addListener((source, reason) => {
  ...
  if (reason === 'canceled_by_user') {
    ...
    void removeTabFromSharedGroup(source.tabId).then(scheduleTabsSync);
  }
});
  ...
  try {
    switch (msg.type) {
      ...
      case 'createTab': {
        ...
        await addTabToSharedGroup(tab.id);
        scheduleTabsSync();
    ...
    switch (msg?.type) {
      ...
      case 'toggleShareTab': {
        ...
        if (await isTabShared(tabId)) {
          ...
          await removeTabFromSharedGroup(tabId);
          scheduleTabsSync();
          sendResponse({ ok: true, shared: false });
        } else {
          await addTabToSharedGroup(tabId);
          scheduleTabsSync();
